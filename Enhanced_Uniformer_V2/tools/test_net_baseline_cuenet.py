...
def perform_test(test_loader, model, test_meter, cfg, writer=None):
    """
    Perform multi-view testing for video classification.
    Computes video-level predictions by averaging softmax scores over multiple clips and crops.
    Logs results and saves predictions to a CSV file.
    """
    ...
    for cur_iter, (inputs, labels, video_idx, meta) in enumerate(test_loader):
        if cfg.NUM_GPUS:
            ...
            # Move labels, video_idx, and meta to GPU
            labels = labels.cuda()
        ...
        if cfg.DETECTION.ENABLE:
            # Fully-convolutional detection forward pass
    ...
    if not cfg.DETECTION.ENABLE:
        ...
        if du.is_root_proc():
            ...
            confidences = all_preds.max(dim=1).values.numpy()  # Fixed

            df = pd.DataFrame({
                "video_index": np.arange(len(all_preds)),
        ...
        logger.info(f"Successfully saved prediction results to {save_path_csv}")
