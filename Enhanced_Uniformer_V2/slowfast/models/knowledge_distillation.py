...
class KnowledgeDistillationModel(nn.Module):
    def __init__(self, cfg):
        ...
        # [MODIFIED] Split classifier for Distillation (CRD) support
        # We separate the feature projection from the final classification head
    ...
    def forward(self, video_tensor=None, pose_tensor=None, input_ids=None, attention_mask=None, return_features=False):
        # [MODIFIED] Added return_features argument
        ...
        # [MODIFIED] Forward pass through split classifier
        features = self.classifier_proj(pooled) # Result is 512-dim vector
        out = self.classifier_head(features)    # Result is Num_classes logits
