...
class TextVisualFusion(nn.Module):
    def __init__(self, dim_visual=400, dim_text=400, hidden=512):
        ...
        # Learnable weight for text contribution
        self.text_weight = nn.Parameter(torch.ones(1))  # scalar weight

        # Normalization for text features
    ...
    def forward(self, visual_feats, text_feats):
        ...
        text_proj = self.text_proj(text_feats)  # (B, hidden)
        text_proj = self.text_norm(text_proj)

        # Apply learnable weight (optionally clamp to prevent extreme growth)
        weighted_text = text_proj * self.text_weight.clamp(0.1, 10.0)

        # Log weight value (for debugging/training monitor)
        if self.training:
...
class BlipFusionCUENet(nn.Module):
