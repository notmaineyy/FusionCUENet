...
class EfficientAdditiveAttnetion(nn.Module):
    ...
    def __init__(self, in_dims=512, token_dim=256, num_heads=2):
        ...
        #self.w_g = nn.Parameter(nn.init.xavier_uniform_(torch.empty(token_dim * num_heads, 1))) 
        self.scale_factor = token_dim ** -0.5
        self.Proj = nn.Linear(token_dim * num_heads, token_dim * num_heads)
        self.final = nn.Linear(token_dim * num_heads, token_dim)

    def forward(self, x, y):
        query = self.to_query(x).permute(1,0,2)
        key = self.to_key(y)
        ...
        #out = out.mean(dim=1).unsqueeze(0)
...
class Extractor2(nn.Module):
    def __init__(
        ...
        #token_dim = d_model // n_head
        #assert token_dim * n_head == d_model, "embed_dim must be divisible by num_heads"

        self.attn = EfficientAdditiveAttnetion(in_dims=d_model, token_dim=d_model, num_heads=n_head)
    ...
    def forward(self, x, y):
        x = x + self.drop_path(self.attn(self.ln_1(x), self.ln_3(y)))
        x = x + self.drop_path(self.mlp(self.ln_2(x)))
        return x
    ...
    def forward(self, x):
        ...
        for i, resblock in enumerate(self.resblocks):
            ...
            if i in self.return_list:
                ...
                tmp_x = tmp_x.reshape(N, T_down*L, C)
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        backbone_drop_path_rate=0., use_checkpoint=False, checkpoint_num=[0], 
        t_size=8, kernel_size=3, dw_reduction=1.5, temporal_downsample=True,
